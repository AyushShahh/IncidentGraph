...
flowchart TD
    ...
    BATCH_EMB --> QDR_SIM{"6. Qdrant Similarity Search (score >= 0.85)"}
...
flowchart TD
    subgraph Discovery ["1. Dynamic Discovery"]
        ...
        DISC -.->|Ignore| IGN[".git, venv, pycache"]
    end
    ...
    subgraph Parsing ["2. AST Parsing & Chunking"]
        ...
        PARSER -->|Route Decorators| ROUTES["Exposed HTTP Routes"]
        PARSER -->|HTTP and Kafka Calls| OUT_CALLS["Outbound Calls"]
    ...
    subgraph State ["3. Indexing & Freshness"]
        ...
        SYM_RAW --> SYMIDX["SymbolIndex (In-Memory Table)"]
        ROUTES --> SYMIDX
        OUT_CALLS --> DGRAPH["DependencyGraph (NetworkX DiGraph)"]
        ROUTES --> DGRAPH
    end

    subgraph Stage4 ["4. Deterministic Retrieval & Context Assembly"]
        SYMIDX --> TOOLS["14 Retrieval Tools (RepositoryTools)"]
        DGRAPH --> TOOLS
        VEC --> TOOLS
        TOOLS --> CTX_BLD["ContextBuilder"]
        CTX_BLD -->|Trace Parsing + Snippets + Blast Radius| PKG["ContextPackage<br/>(Token Budget max 3500)"]
    end
```
...
flowchart TD
    ...
    subgraph Preparation ["Freshness & Context Preparation"]
        ...
        CB["Stage 3 Context Builder<br/>(Deterministic Package max 1500 tokens)"]
    end

    subgraph LangGraphLoop ["LangGraph Autonomous Investigation Loop"]
        Plan["1. Planner Node<br/>(Surgically selects next tool)"]
        Exec["2. Evidence Gathering Node<br/>(Executes Stage 3 retrieval tool)"]
        Prune["Context Manager<br/>(Summarizes output, deduplicates)"]
        Hypo["3. Hypothesis Generator Node<br/>(Synthesizes evidence & confidence)"]
        LoopCheck{"Confidence >= 0.85<br/>or Max Iterations (5)?"}
        Review["4. Reviewer Node<br/>(Adversarial audit & remediation plan)"]
    end
...
The platform includes a dedicated containerized test runner (`test-runner`) executing comprehensive unit and integration test suites:
...
- **Unit Tests (79 passing)**: Test tracing middleware, Kafka producers, log normalizers, HDBSCAN clustering, AST repository indexer, NetworkX dependency graph, LangGraph state nodes, token context bounds, and duplicate check guards.
- **Integration Tests (24 passing)**: Test real multi-service Kafka log pipeline, Redis active fingerprint caching, Qdrant vector retrieval, and LangGraph agent execution.

---

## 4. Realistic Production Traffic Simulator

The traffic simulator triggers genuine runtime failures without artificial injectors:

```bash
# Run all traffic scenarios (normal checkouts and real bug edge cases)
python scripts/generate_traffic.py --scenario all

# Run specific edge case scenarios
python scripts/generate_traffic.py --scenario currency_lookup_error    # Payments KeyError (GBP)
python scripts/generate_traffic.py --scenario customer_tier_error      # Orders KeyError (platinum)
python scripts/generate_traffic.py --scenario inventory_batch_overflow # Inventory IndexError (bulk order)
python scripts/generate_traffic.py --scenario zero_division            # Gateway ZeroDivisionError (promo)
python scripts/generate_traffic.py --scenario payment_declines         # Business payment decline
```
